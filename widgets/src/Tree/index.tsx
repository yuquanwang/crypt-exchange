...
const TreeItem = (CustomFoldIcon: React.ReactType) => function<T> ({ data, dataTypes, level, indent, }: TreeItemProps<T>) {
    ...
    <div>
      <div style={current.style ? Object.assign(fixedStyle, current.style) : fixedStyle}>
        <div style={{ visibility: hasChildren ? 'visible' : 'hidden' }}>
          {
            CustomFoldIcon
              ? <CustomFoldIcon onToggle={(val: boolean) => setShowChildren(val)} />
              : <FoldIcon onToggle={(val: boolean) => setShowChildren(val)} />
          }
        </div>
        <div style={{ flexGrow: 1 }}>{ current.render(data) }</div>
      </div>
      { next && showChildren && <Tree data={data[next.dataIndex]} dataTypes={dataTypes.slice(1)} level={level + 1} indent={indent} foldIcon={CustomFoldIcon} /> }
    </div>
  );
};

function Tree<T>({ data, dataTypes, indent = '30px', level = 0, foldIcon }: TreeProps<T>) {
  const TreeItemWithFoldIcon = TreeItem(foldIcon);
  return (
    <TreeBody level={level} style={{ flexShrink: 0, width: '100%' }} indent={indent}>
      { data.map((item: T, index: number) => (
          <TreeItemWithFoldIcon key={index} data={item} dataTypes={dataTypes} level={level} indent={indent} />
      )) }
    </TreeBody>
  );
}
