...
export const Simple = () => {
  ...
  return (
    <Book>
      <Tree data={addressData} dataTypes={dataTypes} />
    </Book>
  )
}
...
const CustomFoldIcon = ({ onToggle }: { onToggle: Function }) => {
  const [show, setShow] = useState<boolean>(false);
  const handleClick = () => {
    setShow(!show);
    onToggle(!show);
  };
  return <MyIcon onClick={handleClick}> { show ? '-' : '+' } </MyIcon>
}
...
export default { title: 'Tree', decorators: [withKnobs] }
